...
export function middleware(request: NextRequest) {
  ...
  if (refreshToken) {
    ...
    // Nhưng chỉ redirect nếu token còn hợp lệ
    if (unAuthPaths.some((path) => pathname.startsWith(path))) {
      // Cho phép vào login nếu có query param clearTokens
      if (searchParams.get('clearTokens') === 'true') {
        return response
      }
      // Cho phép vào login nếu có accessToken trong query (OAuth redirect)
      if (
      ...
      // Chỉ redirect nếu có cả accessToken và refreshToken hợp lệ
      // Nếu không có accessToken, có thể token đã hết hạn, cho phép vào login để logout
      if (!accessToken) {
        // Không có accessToken, cho phép vào login để logout/clear tokens
        return response
      }
      ...
      try {
        ...
        // Nếu refreshToken còn hợp lệ và có accessToken, thì redirect
        if (decoded && decoded.exp > now) {
          return NextResponse.redirect(new URL(`/${locale}`, request.url))
        }
        // Nếu token hết hạn, cho phép vào login
      } catch (error) {
        // Token không hợp lệ, cho phép vào login
      }
    }
